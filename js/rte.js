...
/**
 * @var COMMON    a JavaScript namespace containing the functionality
...
var COMMON = {
    ...
    validate: function () {
        ...
        $elm = this.$ajaxDialog.find('table.rte_table input[name=item_name]');
        if ($elm.val() === '') {
            $elm.focus();
            alert(PMA_messages['strFormEmpty']);
            return false;
        }
        $elm = this.$ajaxDialog.find('table.rte_table textarea[name=item_definition]');
        if ($elm.val() === '') {
            ...
            if (data.success === true) {
                ...
                that.buttonOptions[PMA_messages['strGo']] = function () {
                    ...
                    if (that.validate()) {
                         ...
                         */
                        var data = that.$ajaxDialog.find('form.rte_form').serialize();
                        $msg = PMA_ajaxShowMessage(PMA_messages['strProcessingRequest']);
                        $.post(that.$ajaxDialog.find('form.rte_form').attr('action'), data, function (data) {
                            if (data.success === true) {
                 ...
                 */
                var mode = 'add';
                if (that.$ajaxDialog.find('input[name=editor_process_edit]').length > 0) {
                    mode = 'edit';
                }
                 ...
                 */
                var $elm = that.$ajaxDialog.find('textarea[name=item_definition]').last();
                /**
                 * @var    opts    Options to pass to the codemirror editor.
                 */
                var opts = {lineNumbers: true, matchBrackets: true, indentUnit: 4, mode: "text/x-mysql"};
 ...
 * @see $cfg['AjaxEnable']
 *
 * TODO: submit on ENTER keypress in dialog
 */
$(document).ready(function () {
    ...
    $('select[name=item_returntype]').live('change', function () {
        var rte = new RTE('routine');
        rte.setOptionsForParameter(
            $(this).closest('table.rte_table').find('select[name=item_returntype]'),
            $(this).closest('table.rte_table').find('input[name=item_returnlength]'),
            $(this).closest('table.rte_table').find('select[name=item_returnopts_text]'),
            $(this).closest('table.rte_table').find('select[name=item_returnopts_num]')
        );
    }); // end $.live()
